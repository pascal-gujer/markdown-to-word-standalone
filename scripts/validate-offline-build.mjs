...
export function neuterAnchorHrefs(html) {
...
export function findOfflineBuildIssues(html) {
  const scanned = neuterAnchorHrefs(html);
  return checks
    .map((check) => ({ ...check, match: scanned.match(check.pattern) }))
...
async function runCli() {
  ...
  const failures = findOfflineBuildIssues(html);
